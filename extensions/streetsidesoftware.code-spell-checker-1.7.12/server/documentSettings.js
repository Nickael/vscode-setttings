...
const defaultRootUri = vscode_uri_1.default.file('').toString();
class DocumentSettings {
    ...
    async isExcluded(uri) {
        ...
        for (const fn of fnExclTests) {
            if (fn(vscode_uri_1.default.parse(uri).path)) {
                return true;
            }
        }
        return false;
    ...
    async findMatchingFolder(docUri) {
        const root = vscode_uri_1.default.parse(docUri || defaultRootUri).with({ path: '' });
...
function doesUriMatchAnyScheme(uri, schemes) {
    const schema = vscode_uri_1.default.parse(uri).scheme;
