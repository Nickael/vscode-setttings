 ...
 * @param scope the scope of the value. A resource is needed to get folder level settings.
 */
function findScopedSettingFromVSConfig(subSection, scope) {
    scope = normalizeScope(scope);
    const ins = inspectSettingFromVSConfig(subSection, scope.resource);
    return findBestConfig(ins, scope.scope);
}
exports.findScopedSettingFromVSConfig = findScopedSettingFromVSConfig;
...
function isGlobalLevelTarget(target) {
    return isConfigTargetWithOptionalResource(target) && target.target === vscode_1.ConfigurationTarget.Global || target === vscode_1.ConfigurationTarget.Global;
}
exports.isGlobalLevelTarget = isGlobalLevelTarget;
...
function extractScope(inspectScope) {
...
function updateCachedSettings(target, section, value) {
    const key = targetToFolderSettingsKey(target);
    const s = cachedSettings.get(key) || {};
    s[section] = value;
...
vscode_1.workspace.onDidChangeConfiguration(event => {
    if (event.affectsConfiguration(sectionCSpell)) {
        cachedSettings.delete(scopeToFolderSettingsKey(exports.Scopes.Global));
        cachedSettings.delete(scopeToFolderSettingsKey(exports.Scopes.Workspace));
    }
